...
class CNN_Discriminator(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        conv_size = out.shape[-1] # 4
        num_features = out.shape[1] # 256
    ...
    def forward(self, z):
        ...
        if z.dtype != torch.complex64 and z.shape[-1] == 2:
            z = torch.view_as_complex(z) # make z complex by combining Re and Im
...
if __name__ == "__main__":
    ...
    img_size = cfg_disc["PARAMETERS"]["image_size"]
    discriminator = eval(cfg_disc["NAME"])(**cfg_disc["PARAMETERS"])

    # Test
    discriminator.to(args.device)
    x = torch.rand(2, 1, img_size, img_size, dtype=torch.complex64).to(args.device) # torch.Size([2, 1, 256, 256])
    y = discriminator(x)
    print(y)

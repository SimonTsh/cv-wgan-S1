...
class C_UpsampleConv2d(nn.Module):
    ...
    def __init__(
        ...
        self.up = lambda x: torch.nn.functional.interpolate(
            x, scale_factor=self.scale_factor, mode=self.upsampling_mode
        )

    def forward(self, z: torch.Tensor):
        upsampled = torch.view_as_complex(
            torch.cat(
                (
                    torch.unsqueeze(self.up(z.real), -1),
...
class C_BatchNorm2d(nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer(
            "sigma_running",
            torch.eye(2) / torch.sqrt(torch.tensor(2)).tile(num_features, 1, 1),
        )
        self.register_buffer(
            "sigma", torch.eye(2) / torch.sqrt(torch.tensor(2)).tile(num_features, 1, 1)
        )
        # [C, 2]
        self.register_buffer(
            "mu_running",
            torch.zeros(1, 2).tile(num_features, 1).reshape(1, num_features, 1, 1, 2),
        )
        self.register_buffer(
            "mu",
            torch.zeros(1, 2).tile(num_features, 1).reshape(1, num_features, 1, 1, 2),
        ...
        self.gamma = torch.nn.Parameter(
            torch.eye(2) / torch.sqrt(torch.tensor(2)), requires_grad=True
        )
        self.beta = torch.nn.Parameter(torch.zeros(2), requires_grad=True)
    ...
    def update_mu_sigma(self, z: torch.Tensor):
        ...
        # [C, 2, 2]
        self.sigma = (
            1 / (B * H * W) * prods.sum(dim=(0, 2, 3)).squeeze()
            + self.id2.tile(C, 1, 1) * self.eps
        )
    ...
    def normalize(self, z: torch.Tensor):
        ...
        z_centered = (z - self.mu).reshape(B * C * H * W, 2).unsqueeze(-1)
        o = torch.bmm(
            sigma_inv_sqrt.tile(B * H * W, 1, 1).reshape(B * C * H * W, 2, 2),
            z_centered,
        )
        return o
    ...
    def normalize_inference(self, z: torch.Tensor):
        ...
        # [B, C, H, W, 2, 1]
        z_centered = (
            (z - self.mu_running.reshape(1, C, 1, 1, 2))
            .reshape(B * C * H * W, 2)
            .unsqueeze(-1)
        )
        o = torch.bmm(
...
def get_convolution_block(
    ...
    if kernel_size_pool != 0:
        pool_layer = C_MaxPool2d(kernel_size_pool, stride_pool, padding_pool)
